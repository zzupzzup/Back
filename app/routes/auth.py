...
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email: # email 의 경우 # 진실이 -> 회원가입 창 만들라고 하기
        is_exist = await is_email_exist(reg_info.email)
        if not reg_info.email or not reg_info.pw: # email 과 pw 가 공란일 경우
            return JSONResponse(status_code=400, content=dict(msg="Email and PW must be provided'"))
        if is_exist: # 이미 기존에 존재하는 email 인 경우 
            return JSONResponse(status_code=400, content=dict(msg="EMAIL_EXISTS"))
        hash_pw = bcrypt.hashpw(reg_info.pw.encode("utf-8"), bcrypt.gensalt()) # pw 해시해서 저장할 목적
        new_user = Users.create(session, auto_commit=True, pw=hash_pw, email=reg_info.email) # email 과 pw 를 DB 에 추가
